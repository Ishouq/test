...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["username"])) {
      //cheack of the feild user name not empty , if empty we tell the admin the user name is required
    $usernameErr = "username is required";
  } else {
      //if user name nut empty cheack the value in feild
    $username = test_input($_POST["username"]);
    if (!preg_match("/^[a]{1}+[0-9]{6}$/",$username) ){
      $usernameErr = "Just letter a and 6 numbers allowed";
    }
    else{
        $usernameFlag = TRUE;
    }
  ...
  if($usernameFlag==true&&$passwordFlag==true){
      if (isset($_POST['login'])){
            ...
            if (!empty ($User)&&($Pwd)){  
                //cheack if the admin username and pass word not empty and stored in the db
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    //if log info for admin is true and choose log in we show the admin home page for admin
                    print '<meta http-equiv="refresh" content="0.5; URL=AdminHome.php">';
                    $sql1="UPDATE university SET state='on' WHERE ID='$User'";
                      $result1 = $conn->query($sql1);
                }
            } else {
            //if admin choose log in without inter his username and password we show the 
